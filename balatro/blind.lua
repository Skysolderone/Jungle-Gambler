--
-- 文件名：blind.lua
-- 作用：定义Blind类，实现盲注的属性、颜色、文本、奖励、特效等。
-- 主要用于管理不同盲注的显示、切换、奖励和特殊效果。
--
...
function Blind:init(X, Y, W, H)
    ...
    self.children.animatedSprite = AnimatedSprite(self.T.x, self.T.y, self.T.w, self.T.h,
        G.ANIMATION_ATLAS['blind_chips'], G.P_BLINDS.bl_small.pos)
...
--[[
函数名：Blind:change_colour
作用：更改盲注的主色调和暗色调。
参数：blind_col（table，可选）：颜色表
返回：无
]]
...
--[[
函数名：Blind:set_blind
作用：设置盲注的属性、奖励、状态、颜色等。
参数：blind（table）：盲注原型；reset（bool，可选）：是否重置；silent（bool，可选）：是否静默
返回：无
]]
function Blind:set_blind(blind, reset, silent)
                    ...
                    G.E_MANAGER:add_event(Event({
                        ...
                        func = (function()
                            ...
                            if not reset and not silent then
                                self:juice_up()
                                if blind then
                                    play_sound('chips1', math.random() * 0.1 + 0.55, 0.42); play_sound('gold_seal',
                                        math.random() * 0.1 + 1.85, 0.26)                                                                                  --play_sound('cancel')
                                end
                            end
                            return true
                        end)
    ...
    (self.config.blind.boss and (self.config.blind.boss.showdown and 0.5 or 0.25) or 0)
end
...
function Blind:alert_debuff(first)
    if self.loc_debuff_text and self.loc_debuff_text ~= '' then
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = (function()
                if self.disabled then
                    self.block_play = nil; return true
                end
                if G.STATE == G.STATES.SELECTING_HAND then
                    G.E_MANAGER:add_event(Event({
                        ...
                        func = (function()
                            ...
                            attention_text({
                                scale = 0.7, text = disp_text, maxw = 12, hold = hold_time, align = 'cm', offset = { x = 0, y = -1 }, major =
                            G.play
                            })
                            G.E_MANAGER:add_event(Event({
                                trigger = 'after',
...
function Blind:get_loc_debuff_text()
    ...
    self.loc_debuff_text
    if (self.config.blind.name == 'The Mouth') and self.only_hand then disp_text = disp_text ..
        ' [' .. localize(self.only_hand, 'poker_hands') .. ']' end
    return disp_text
end
    ...
    if not silent then
        for i = 1, math.min(self.sound_pings or 3, 7) do
            ...
            G.E_MANAGER:add_event(Event({
                blockable = false,
                trigger = 'after',
                delay = (0.15 - 0.01 * (self.sound_pings or 3)) * i * dissolve_time,
                func = function()
...
function Blind:load(blindTable)
    ...
    G.ARGS.spin.real = (G.SETTINGS.reduced_motion and 0 or 1) *
    (self.config.blind.boss and (self.config.blind.boss.showdown and 1 or 0.3) or 0)
