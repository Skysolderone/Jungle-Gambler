--
-- 文件名：back.lua
-- 作用：定义Back类，实现卡背的属性、UI生成、切换、存档、特效等。
-- 主要用于管理不同卡背的解锁、显示、切换和应用效果。
--
...
--[[
函数名：Back:generate_UI
作用：生成卡背的UI描述信息，包含解锁条件、效果等。
参数：other（table，可选）：其他卡背原型；ui_scale（number，可选）：缩放；min_dims（number，可选）：最小尺寸；challenge（string，可选）：挑战ID
返回：table：UI定义表
]]
function Back:generate_UI(other, ui_scale, min_dims, challenge)
    ...
    return
    {
        ...
        nodes = {
            name_to_check == 'Challenge Deck' and
            UIBox_button({ button = 'deck_view_challenge', label = { localize(challenge.id, 'challenge_names') }, minw = 2.2, minh = 1, scale = 0.6, id =
            challenge })
...
function Back:apply_to_run()
    ...
    if self.effect.config.consumable_slot then
        ...
        self.effect.config.consumable_slot
    end
