--
-- 文件名：main.lua
-- 作用：游戏主入口，负责初始化、主循环、事件分发、窗口管理、Steam集成、错误处理等。
-- 依赖Love2D引擎，加载各类模块，管理游戏生命周期。
--
...
--[[
函数名：love.run
作用：Love2D主循环函数，负责事件分发、帧率控制、调用update/draw等核心流程。
参数：无
返回：主循环闭包函数
]]
function love.run()
...
--[[
函数名：love.load
作用：游戏启动时调用，初始化全局对象、Steam集成、设置鼠标可见性等。
参数：无
返回：无
]]
function love.load()
...
--[[
函数名：love.keyreleased
作用：键盘释放事件处理，支持开发模式下的按键映射和普通按键。
参数：key（string）：释放的键名
返回：无
]]
...
--[[
函数名：love.gamepadpressed
作用：手柄按键按下事件处理。
参数：joystick（对象）：手柄对象；button（string）：按下的按钮名
返回：无
]]
...
--[[
函数名：love.gamepadreleased
作用：手柄按键释放事件处理。
参数：joystick（对象）：手柄对象；button（string）：释放的按钮名
返回：无
]]
...
--[[
函数名：love.mousepressed
作用：鼠标按下事件处理，区分左右键和触摸。
参数：x（number）：横坐标；y（number）：纵坐标；button（number）：按键编号；touch（bool）：是否为触摸
返回：无
]]
...
--[[
函数名：love.mousereleased
作用：鼠标释放事件处理。
参数：x（number）：横坐标；y（number）：纵坐标；button（number）：按键编号
返回：无
]]
...
函数名：love.mousemoved
作用：鼠标移动/触摸移动事件处理。
参数：x（number）：横坐标；y（number）：纵坐标；dx（number）：横向变化；dy（number）：纵向变化；istouch（bool）：是否为触摸
返回：无
]]
...
--[[
函数名：love.joystickaxis
作用：手柄摇杆轴移动事件处理。
参数：joystick（对象）：手柄对象；axis（number）：轴编号；value（number）：轴值
返回：无
]]
...
function love.errhand(msg)
	...
	if G.SETTINGS.crashreports and _RELEASE_MODE and G.F_CRASH_REPORTS then
		...
		'&file=' ..
		httpencode(file) ..
		'&function_line=' .. httpencode(function_line) .. '&trace=' .. httpencode(trace) .. '&version=' .. (G.VERSION))
	end
...
--[[
函数名：love.resize
作用：窗口尺寸变化时调用，重新计算画布和UI布局。
参数：w（number）：新宽度；h（number）：新高度
返回：无
]]
