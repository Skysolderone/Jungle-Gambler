# 战斗系统改进方案 - 借鉴 Balatro 设计

> 基于 Balatro 游戏战斗系统的深度分析
> 目标：将 Jungle Gambler 的战斗系统升级为更有策略性和趣味性的回合制卡牌战斗

---

## 目录

1. [当前系统分析](#1-当前系统分析)
2. [Balatro 核心设计借鉴](#2-balatro-核心设计借鉴)
3. [改进方案设计](#3-改进方案设计)
4. [实现路线图](#4-实现路线图)
5. [代码架构设计](#5-代码架构设计)

---

## 1. 当前系统分析

### 1.1 现有战斗流程

```
当前流程：
准备阶段 (10秒) → 选择魂印
	↓
战斗阶段 → 自动回合制
	- 掷骰子（共用）
	- 计算力量 = 基础力量 × 骰子 + 魂印加成
	- 伤害 = abs(玩家力量 - 敌人力量)
	- 重复直到一方 HP 归零
	↓
战利品阶段 → 背包满时选择丢弃
```

### 1.2 当前系统的优点

✅ **简洁直观** - 规则简单易懂
✅ **快节奏** - 战斗自动进行
✅ **资源管理** - 魂印有使用次数限制
✅ **战利品系统** - 背包满时需要取舍

### 1.3 当前系统的问题

❌ **缺乏策略深度** - 玩家只在准备阶段做决策
❌ **战斗过程单调** - 无法控制战斗流程
❌ **随机性过高** - 完全依赖骰子运气
❌ **魂印同质化** - 只有数值差异，无特殊效果
❌ **缺少反馈** - 战斗过程缺少视觉和音效反馈
❌ **无法中途调整** - 战斗开始后只能观看

---

## 2. Balatro 核心设计借鉴

### 2.1 状态机架构

Balatro 使用清晰的状态机管理战斗流程：

```
状态机模式：
SELECTING_HAND (选牌)
	↓ 玩家选择卡牌
HAND_PLAYED (出牌)
	↓ 动画和计算
ROUND_EVAL (评估)
	↓ 检查胜利条件
回到 SELECTING_HAND 或 结束
```

**可借鉴点**：
- 每个状态职责单一
- 状态之间转换清晰
- 玩家在每回合都有决策空间

### 2.2 资源管理系统

```gdscript
# Balatro 的资源管理
hands_left = 4        # 出牌次数
discards_left = 3     # 弃牌次数
target_chips = 600    # 目标分数
```

**可借鉴点**：
- 多种资源类型增加策略深度
- 弃牌机制允许优化手牌
- 明确的胜利目标

### 2.3 分层计算系统

```
计分公式：
Base → Enhancement → Card Effects → Hand Type → Joker → Blind → Final

例如：
10 chips × 2 mult
+ Card Enhancement (+5 chips, +3 mult)
+ Joker Effects (×1.5 mult)
+ Hand Modifiers (+20 chips, ×2 mult)
= 最终分数
```

**可借鉴点**：
- 多层修改器可以无限扩展
- 效果叠加顺序清晰
- 易于调试和平衡

### 2.4 CardArea 系统

```gdscript
# 卡牌区域管理
CardArea:
	- cards: Array[Card]          # 卡牌列表
	- highlighted: Array[Card]    # 选中的卡牌
	- highlighted_limit: int      # 最多选择数量
	- align_cards()               # 自动布局
```

**可借鉴点**：
- 自动处理卡牌布局和间距
- 统一的卡牌容器管理
- 高亮和选择状态分离

### 2.5 事件驱动动画

```gdscript
# Balatro 的事件系统
Event:ease(duration, target, ease_func)
Event:after(delay, callback)
Event:condition(check_func, callback)
```

**可借鉴点**：
- 异步动画不阻塞逻辑
- 事件队列管理复杂序列
- 易于添加视觉反馈

---

## 3. 改进方案设计

### 3.1 新战斗流程设计

```
改进后的流程：

═══════════════════════════════════════
  阶段 1: 准备阶段 (PREPARATION)
═══════════════════════════════════════
- 时间限制：15 秒（增加 5 秒）
- 玩家行动：
  ✓ 选择魂印组合（最多 5 个）
  ✓ 查看魂印详情和效果
  ✓ 配置魂印使用顺序
- 敌人行动：显示敌人魂印和能力

═══════════════════════════════════════
  阶段 2: 战斗回合 (COMBAT_ROUNDS)
═══════════════════════════════════════

【回合开始】
  ↓
【玩家回合】(PLAYER_TURN)
  - 可用次数：4 次出手（hands）
  - 每次出手：
	1. 选择要使用的魂印（1-3 个）
	2. 掷骰子
	3. 计算伤害 = (基础力量 + 魂印力量总和) × 骰子
	4. 对敌人造成伤害
  - 弃牌选项：2 次机会（discards）
	* 可以弃掉不想用的魂印
	* 弃掉的魂印不消耗使用次数
  ↓
【敌人回合】(ENEMY_TURN)
  - 敌人自动使用魂印
  - 计算并对玩家造成伤害
  ↓
【回合结束检查】
  - 检查是否有一方 HP 归零
  - 如果都存活且玩家还有出手次数 → 下一回合
  - 如果出手次数用尽且未击败敌人 → 失败
  ↓
重复或结束

═══════════════════════════════════════
  阶段 3: 战利品阶段 (LOOT)
═══════════════════════════════════════
- 与现有系统保持一致
- 增加战利品预览动画
```

### 3.2 新状态机设计

```gdscript
enum BattleState {
	PREPARATION,        # 准备阶段
	PLAYER_TURN,        # 玩家回合
	PLAYER_SELECTING,   # 玩家选择魂印
	PLAYER_ROLLING,     # 玩家掷骰子
	PLAYER_ATTACKING,   # 玩家攻击动画
	ENEMY_TURN,         # 敌人回合
	ENEMY_ATTACKING,    # 敌人攻击动画
	ROUND_END,          # 回合结束
	VICTORY,            # 战斗胜利
	DEFEAT,             # 战斗失败
	LOOT                # 战利品
}
```

### 3.3 魂印效果系统

引入 Balatro 风格的效果层系统：

```gdscript
# 魂印效果类型
enum SoulEffectType {
	POWER_FLAT,      # 固定力量加成 +10
	POWER_MULT,      # 力量倍数 ×1.5
	DICE_BONUS,      # 骰子加成 +2
	DICE_REROLL,     # 重掷骰子
	SHIELD,          # 格挡伤害
	LIFE_STEAL,      # 生命偷取
	COMBO_BONUS,     # 连击加成
	CRIT_CHANCE      # 暴击几率
}

# 魂印定义示例
{
	"id": "soul_thunder",
	"name": "雷霆之魂",
	"quality": 2,
	"power": 15,
	"effects": [
		{
			"type": POWER_FLAT,
			"value": 15
		},
		{
			"type": DICE_BONUS,
			"value": 1,
			"condition": "dice >= 4"  # 骰子≥4时生效
		}
	]
}
```

### 3.4 UI 布局改进

```
新战斗界面布局：

┌─────────────────────────────────────────────┐
│  [战斗信息栏]                                │
│  回合: 1  |  出手次数: 3/4  |  弃牌: 2/2    │
│  目标: 击败敌人  |  时间: 15s               │
└─────────────────────────────────────────────┘

┌──────────────────┐  ┌──────────────────────┐
│   敌人信息        │  │   玩家信息            │
│   HP: 80/100     │  │   HP: 90/100         │
│   力量: 30       │  │   力量: 50           │
│   [敌人魂印区]    │  │   选中魂印力量: +25   │
└──────────────────┘  └──────────────────────┘

┌─────────────────────────────────────────────┐
│         [战斗动画区域]                       │
│         骰子、攻击特效、伤害数字             │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│         [玩家魂印手牌区]                     │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐      │
│  │魂印│ │魂印│ │魂印│ │魂印│ │魂印│      │
│  │ 1  │ │ 2  │ │ 3  │ │ 4  │ │ 5  │      │
│  └────┘ └────┘ └────┘ └────┘ └────┘      │
│  [点击选择要使用的魂印，最多 3 个]           │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│  [操作按钮区]                                │
│  [出手] [弃牌] [跳过回合] [查看详情]         │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│  [战斗日志]                                  │
│  > 玩家掷出骰子: 5                           │
│  > 使用魂印: 火焰之魂 (+20)                  │
│  > 最终力量: 50 + 20 = 70 × 5 = 350        │
│  > 对敌人造成 350 点伤害！                   │
└─────────────────────────────────────────────┘
```

---

## 4. 实现路线图

### 阶段 1: 核心架构重构 (优先级: 高)

**任务**：
- [ ] 创建新的状态机系统 `BattleStateMachine.gd`
- [ ] 实现状态转换管理器
- [ ] 创建魂印效果系统 `SoulEffect.gd`
- [ ] 重构 UI 布局为模块化组件

**预计时间**：2-3 小时

### 阶段 2: 回合制系统实现 (优先级: 高)

**任务**：
- [ ] 实现玩家回合逻辑（选择、出手、弃牌）
- [ ] 实现敌人 AI 回合逻辑
- [ ] 添加出手次数和弃牌次数管理
- [ ] 实现回合结束检查

**预计时间**：3-4 小时

### 阶段 3: 魂印选择和战斗动画 (优先级: 中)

**任务**：
- [ ] 实现魂印卡片选择系统（类似 Balatro 的高亮系统）
- [ ] 添加骰子滚动动画
- [ ] 添加伤害数字飘出动画
- [ ] 添加魂印激活特效

**预计时间**：2-3 小时

### 阶段 4: 魂印效果系统 (优先级: 中)

**任务**：
- [ ] 实现多种魂印效果类型
- [ ] 添加效果触发条件系统
- [ ] 实现效果计算器（分层计算）
- [ ] 更新魂印数据库添加新效果

**预计时间**：3-4 小时

### 阶段 5: 音效和反馈优化 (优先级: 低)

**任务**：
- [ ] 添加战斗音效
- [ ] 添加 UI 反馈音效
- [ ] 优化动画流畅度
- [ ] 添加屏幕震动效果

**预计时间**：1-2 小时

---

## 5. 代码架构设计

### 5.1 新文件结构

```
scripts/
├── battle/
│   ├── BattleStateMachine.gd       # 状态机管理
│   ├── BattleState.gd              # 状态基类
│   ├── PreparationState.gd         # 准备阶段
│   ├── PlayerTurnState.gd          # 玩家回合
│   ├── EnemyTurnState.gd           # 敌人回合
│   ├── LootState.gd                # 战利品阶段
│   ├── SoulEffect.gd               # 魂印效果系统
│   ├── DamageCalculator.gd         # 伤害计算器
│   └── BattleAnimator.gd           # 战斗动画管理
├── Battle.gd                       # 主战斗场景（简化）
└── ...
```

### 5.2 核心类设计

#### BattleStateMachine.gd

```gdscript
extends Node
class_name BattleStateMachine

signal state_changed(from_state, to_state)

var current_state: BattleState
var states: Dictionary = {}

func _ready():
	# 注册所有状态
	register_state("preparation", PreparationState.new())
	register_state("player_turn", PlayerTurnState.new())
	register_state("enemy_turn", EnemyTurnState.new())
	register_state("loot", LootState.new())

func register_state(name: String, state: BattleState):
	states[name] = state
	state.state_machine = self

func change_state(state_name: String):
	if current_state:
		current_state.exit()

	var old_state = current_state
	current_state = states[state_name]
	current_state.enter()

	state_changed.emit(old_state, current_state)
```

#### BattleState.gd (抽象基类)

```gdscript
extends Node
class_name BattleState

var state_machine: BattleStateMachine
var battle_scene: Node  # 主战斗场景引用

func enter():
	# 进入状态时调用
	pass

func exit():
	# 退出状态时调用
	pass

func update(delta: float):
	# 每帧更新
	pass

func handle_input(event: InputEvent):
	# 处理输入
	pass
```

#### SoulEffect.gd

```gdscript
extends Resource
class_name SoulEffect

enum EffectType {
	POWER_FLAT,      # 固定力量 +X
	POWER_MULT,      # 力量倍数 ×X
	DICE_BONUS,      # 骰子加成 +X
	DICE_REROLL,     # 重掷骰子
	SHIELD,          # 格挡伤害
	LIFE_STEAL,      # 生命偷取 X%
	COMBO_BONUS,     # 连击加成
	CRIT_CHANCE      # 暴击几率 X%
}

@export var type: EffectType
@export var value: float
@export var condition: String = ""  # 触发条件（可选）

func can_apply(context: Dictionary) -> bool:
	"""检查效果是否可以应用"""
	if condition.is_empty():
		return true

	# 简单条件解析
	# 例如: "dice >= 4", "hp < 50", "combo >= 3"
	return _evaluate_condition(condition, context)

func apply(context: Dictionary) -> Dictionary:
	"""应用效果并返回修改后的上下文"""
	match type:
		EffectType.POWER_FLAT:
			context["power"] += value
		EffectType.POWER_MULT:
			context["power"] *= value
		EffectType.DICE_BONUS:
			context["dice"] += value
		# ... 其他效果

	return context

func _evaluate_condition(condition_str: String, context: Dictionary) -> bool:
	# 条件解析逻辑
	# 可以使用简单的字符串解析或表达式求值
	pass
```

#### DamageCalculator.gd

```gdscript
extends Node
class_name DamageCalculator

# 借鉴 Balatro 的分层计算
func calculate_damage(base_power: int, dice: int, souls: Array, context: Dictionary) -> int:
	"""
    分层计算伤害：
    1. 基础层：基础力量 × 骰子
    2. 魂印平值层：所有 POWER_FLAT 效果
    3. 魂印倍数层：所有 POWER_MULT 效果
    4. 特殊效果层：暴击、连击等
	"""

	var result = {
		"base_power": base_power,
		"dice": dice,
		"power": base_power,
		"multiplier": 1.0,
		"bonus": 0,
		"context": context
	}

	# 阶段 1: 应用骰子加成效果
	for soul in souls:
		for effect in soul.effects:
			if effect.type == SoulEffect.EffectType.DICE_BONUS:
				if effect.can_apply(result):
					result = effect.apply(result)

	# 阶段 2: 基础计算
	result["power"] = result["base_power"] * result["dice"]

	# 阶段 3: 应用固定加成
	for soul in souls:
		for effect in soul.effects:
			if effect.type == SoulEffect.EffectType.POWER_FLAT:
				if effect.can_apply(result):
					result = effect.apply(result)

	# 阶段 4: 应用倍数
	for soul in souls:
		for effect in soul.effects:
			if effect.type == SoulEffect.EffectType.POWER_MULT:
				if effect.can_apply(result):
					result = effect.apply(result)

	# 阶段 5: 特殊效果（暴击等）
	# ... 更多特殊效果

	return int(result["power"])
```

#### PlayerTurnState.gd

```gdscript
extends BattleState
class_name PlayerTurnState

var selected_souls: Array = []
var hands_left: int = 4
var discards_left: int = 2

func enter():
	battle_scene.show_message("你的回合！")
	battle_scene.update_resources_display(hands_left, discards_left)
	battle_scene.enable_soul_selection(true)
	battle_scene.enable_action_buttons(true)

func exit():
	battle_scene.enable_soul_selection(false)
	battle_scene.enable_action_buttons(false)

func on_soul_selected(soul):
	if selected_souls.size() < 3:
		selected_souls.append(soul)
		battle_scene.highlight_soul(soul, true)

func on_soul_deselected(soul):
	selected_souls.erase(soul)
	battle_scene.highlight_soul(soul, false)

func on_play_button_pressed():
	if selected_souls.is_empty():
		battle_scene.show_message("请至少选择一个魂印！")
		return

	if hands_left <= 0:
		battle_scene.show_message("出手次数已用尽！")
		return

	hands_left -= 1
	_execute_attack()

func on_discard_button_pressed():
	if selected_souls.is_empty():
		battle_scene.show_message("请选择要弃掉的魂印！")
		return

	if discards_left <= 0:
		battle_scene.show_message("弃牌次数已用尽！")
		return

	discards_left -= 1
	# 弃掉选中的魂印（不消耗使用次数）
	selected_souls.clear()
	battle_scene.show_message("已弃牌")

func _execute_attack():
	# 播放攻击动画序列
	await battle_scene.play_dice_animation()
	var dice = randi() % 6 + 1

	var damage = battle_scene.damage_calculator.calculate_damage(
		battle_scene.player_base_power,
		dice,
		selected_souls,
		{"attacker": "player"}
	)

	await battle_scene.play_attack_animation(damage)
	battle_scene.enemy_hp -= damage

	selected_souls.clear()
	battle_scene.update_display()

	# 检查战斗是否结束
	if battle_scene.enemy_hp <= 0:
		state_machine.change_state("victory")
	elif hands_left <= 0:
		state_machine.change_state("enemy_turn")
```

---

## 6. 关键改进点总结

### 6.1 借鉴 Balatro 的设计优势

| 设计元素 | Balatro 实现 | 我们的应用 |
|---------|-------------|-----------|
| **状态机** | 19 个清晰状态 | 10 个战斗状态，职责分离 |
| **资源管理** | hands/discards | 出手次数/弃牌次数 |
| **分层计算** | 6 层效果叠加 | 5 层伤害计算 |
| **CardArea** | 自动布局管理 | 魂印卡片区域管理 |
| **事件驱动** | Event 系统 | Godot Signal + Tween |

### 6.2 保留原有系统的优点

✅ **魂印使用次数** - 保留耐久度机制
✅ **战利品选择** - 保留背包管理策略
✅ **品质系统** - 保留魂印品质分级
✅ **简洁快速** - 不过度复杂化

### 6.3 新增策略深度

🎯 **回合内决策** - 每回合选择使用哪些魂印
🎯 **资源管理** - 平衡出手次数和弃牌次数
🎯 **魂印组合** - 不同效果的协同作用
🎯 **风险控制** - 选择保守或激进的策略

---

## 7. 开发优先级建议

### 第一阶段（核心功能）- 必须实现

1. **状态机系统** - 战斗流程的基础
2. **回合制系统** - 玩家和敌人轮流行动
3. **魂印选择 UI** - 清晰的选择反馈
4. **基础动画** - 骰子、伤害数字

### 第二阶段（增强体验）- 推荐实现

5. **魂印效果系统** - 多样化的效果类型
6. **战斗日志优化** - 更清晰的信息展示
7. **音效反馈** - 提升打击感

### 第三阶段（锦上添花）- 可选实现

8. **特效动画** - 华丽的视觉效果
9. **敌人 AI 变化** - 不同的行为模式
10. **战斗统计** - 伤害分析、效率评分

---

## 8. 实现示例代码片段

### 改进后的 Battle.gd 主场景

```gdscript
extends Control

# 子系统
var state_machine: BattleStateMachine
var damage_calculator: DamageCalculator
var battle_animator: BattleAnimator

# 战斗数据
var player_hp: int = 100
var enemy_hp: int = 100
var player_base_power: int = 50
var enemy_base_power: int = 30

func _ready():
	# 初始化子系统
	state_machine = BattleStateMachine.new()
	damage_calculator = DamageCalculator.new()
	battle_animator = BattleAnimator.new()

	add_child(state_machine)
	add_child(damage_calculator)
	add_child(battle_animator)

	# 设置状态机
	state_machine.battle_scene = self

	# 加载战斗数据
	_load_battle_data()

	# 开始准备阶段
	state_machine.change_state("preparation")

func _process(delta):
	if state_machine.current_state:
		state_machine.current_state.update(delta)
```

---

## 9. 测试计划

### 单元测试

- [ ] 状态转换逻辑测试
- [ ] 伤害计算公式测试
- [ ] 魂印效果应用测试
- [ ] 资源管理（次数）测试

### 集成测试

- [ ] 完整战斗流程测试
- [ ] 多种魂印组合测试
- [ ] 边界条件测试（HP 归零、次数用尽等）

### 玩法测试

- [ ] 策略深度评估
- [ ] 难度平衡调整
- [ ] 用户体验反馈

---

## 10. 总结

通过借鉴 Balatro 的优秀设计，我们可以将 Jungle Gambler 的战斗系统升级为：

✨ **更有策略性** - 玩家在每回合都需要做决策
✨ **更有深度** - 魂印效果系统提供丰富的组合
✨ **更有趣味** - 动画和反馈增强游戏体验
✨ **架构更清晰** - 状态机和模块化设计易于维护

同时保留了原有系统的优点，不会让游戏变得过于复杂。

---

**下一步行动**：
1. 审查本方案并确认改进方向
2. 开始实现第一阶段：核心架构重构
3. 逐步迭代，测试和优化

**预计总开发时间**：12-15 小时
**预计完成日期**：根据开发节奏调整
